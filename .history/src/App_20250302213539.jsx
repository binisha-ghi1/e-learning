...
const App = () => {
  ...
  // Check if we're on the login or signup page, in which case don't show the sidebar
  const hideSidebarPages = ["/login", "/signup"].includes(location.pathname);

  // Show Sidebar only on Dashboard and other routes where the user is logged in
  const showSidebar = user && !hideSidebarPages && ["/dashboard"].includes(location.pathname);

  return (
    <div>
      <Navbar cart={cart} wishlist={wishlist} setWishlist={setWishlist} user={user} onLogout={handleLogout} />
      <div className="flex">
        {/* Only show Sidebar on Dashboard page if the user is logged in */}
