...
const App = () => {
  ...
  const handleLogout = () => {
    ...
    auth.signOut()
      .then(() => {
        // Clear user data from localStorage (if you're using it)
  ...
  // Determine if the sidebar should be shown (don't show on /login)
  const showSidebar = [
    "/dashboard", "/profile", "/mycourses", "/tasks", "/resources",
    "/feedback", "/messages", "/settings", "/wishlist"
  ].includes(location.pathname) && user && location.pathname !== '/login'; // Only show the sidebar if the user is logged in and not on login page

  return (
    <div>
      <Navbar cart={cart} wishlist={wishlist} setWishlist={setWishlist} user={user} onLogout={handleLogout} />
      <div className="flex">
        {/* Only show Sidebar if user is logged in and not on the login page */}
