        ...
        <div className='flex flex-col items-start justify-start pl-20'>
          ...
          <div className='flex flex-col bg-gray-100 text-center pt-4 pb-4 pl-4 pr-4'>
            ...
            {['All Courses', 'Free Courses', 'IT Courses', 'Designing Courses', 'Marketing Courses', 'Business Courses', 'Creative Arts Courses'].map(cat => (
              <p key={cat} className='font-mono pb-1 hover:underline cursor-pointer' onClick={() => setCategory(cat)}>{cat}</p>
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {filteredCourses.map((course, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-lg flex flex-col items-center">
              ...
              <p className="text-lg w-22 text-center rounded-full text-blue-950 pb-2 bg-yellow-400 font-bold mt-2"><strong></strong> {course.price}</p>
