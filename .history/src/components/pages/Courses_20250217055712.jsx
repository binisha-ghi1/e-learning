...
function Courses() {
  ...
  const [cart, setCart] = useState(() => {
    // Retrieve the cart from local storage or set it as an empty array if no cart is found
  ...
  const addToCart = (course) => {
    ...
    setCart(updatedCart);
    localStorage.setItem('cart', JSON.stringify(updatedCart)); // Save the updated cart in local storage
  };
        ...
        <div className='flex flex-col items-start justify-start pl-20'>
          ...
          <div className='flex flex-col bg-gray-100 text-center pt-4 pb-6 pl-10 pr-10 rounded-lg shadow-lg shadow-blue-900'>
            ...
            {['All Courses', 'Free Courses', 'IT Courses', 'Designing Courses', 'Marketing Courses', 'Business Courses', 'Creative Arts Courses'].map(cat => (
              <p key={cat} className={`font-medium pb-6 hover:underline cursor-pointer ${category === cat ? 'text-blue-950 font-bold' : ''}`} onClick={() => setCategory(cat)}>
                {cat}
              </p>
            ))}
          </div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {filteredCourses.map((course, index) => (
            <div key={index} className="bg-white p-6 rounded-2xl shadow-lg shadow-blue-900 flex flex-col items-center">
              ...
              <p className="text-lg w-22 text-center rounded-full text-blue-950 mb-2 bg-yellow-400 font-bold mt-2">{course.price}</p>
              ...
              <div className='mt-4 w-full'>
                <button onClick={() => addToCart(course)} className='bg-yellow-400 text-blue-950 py-2 px-6 rounded-full flex items-center justify-center hover:bg-yellow-500 w-full'>
