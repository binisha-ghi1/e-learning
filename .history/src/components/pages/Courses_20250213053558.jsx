...
function Courses() {
  ...
  const courseList = [
    ...
    { name: 'Data Science', duration: '2.5 Months', price: '10,000', instructor: 'Charlotte', category: 'IT Courses', id: 'data-science-charlotte' },
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {filteredCourses.map((course, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-lg">
              ...
              {/* Add to Cart Button */}
              <div className='mt-4'>
                <button className='bg-yellow-400 text-white py-2 px-6 rounded-full w-full text-center hover:bg-yellow-500'>
                  Add to Cart
                </button>
              </div>
              
              {/* View Course Button (Navigation to course detail page) */}
              <div className='mt-5 text-center hover:bg-gray-200 text-xl font-medium border-2 border-blue-900 text-blue-900 h-12'>
                <Link to={`/course/${course.id}`} className='pt-3 cursor-pointer w-full'>
                  VIEW COURSE
                </Link>
              </div>
            </div>
