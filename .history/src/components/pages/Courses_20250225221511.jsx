...
const Courses = ({ cart, setCart, wishlist, setWishlist }) => {
  ...
  return (
    <div className="flex flex-wrap">
      <div className="flex flex-row flex-wrap gap-10 pt-20 min-h-screen bg-gray-200">
        {/* Sidebar */}
        <div className="w-full sm:w-1/4 md:w-1/5 bg-white p-4 shadow-lg rounded-lg">
          <h1 className="text-xl font-extrabold text-center mb-4">OUR COURSES</h1>
          <div className="flex flex-col bg-gray-100 text-center pt-4 pb-6 pl-10 pr-10 rounded-lg shadow-md">
        ...
        {/* Course List */}
        <div className="flex-1 flex-wrap pt-10 pb-8 pr-4 sm:pr-20">
          <div className="max-w-full sm:max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            {filteredCourses.length > 0 ? (
              filteredCourses.map((course) => (
                <div key={course.id} className="bg-white p-6 rounded-2xl shadow-lg flex flex-col items-center">
                  ...
                  <p className="text-lg w-full text-center rounded-full text-blue-950 mb-2 bg-yellow-400 font-bold mt-2">
                  ...
                  <div className="flex items-center justify-center gap-4 mt-4">
                    <button
                      ...
                      className="bg-yellow-400 text-blue-950 py-2 px-6 rounded-full hover:bg-yellow-500 transition duration-300 w-40"
                    >
                      <IoCartOutline className="text-xl mr-2" /> Add to Cart
                    </button>
                    <button
                      onClick={() => addToWishlist(course)}
                      className="text-blue-950 px-6 transition duration-300"
                    >
