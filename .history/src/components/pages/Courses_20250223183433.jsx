...
const Courses = ({ cart, setCart }) => {
    ...
    const handleAddToWishlist = (course) => {
        // Add course to wishlist if it's not already there
        if (!wishlist.some(item => item.id === course.id)) {
          setWishlist([...wishlist, course]);  // Add course to wishlist
        } else {
    ...
    return (
      ...
      <button onClick={() => handleAddToWishlist(course)}>Add to Wishlist</button>
