  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12">
      <div className="max-w-4xl mx-auto px-4">
        <h2 className="text-4xl font-bold text-blue-950 mb-8 text-center">Your Cart</h2>
        
        {cart.length === 0 ? (
          <div className="bg-white rounded-lg p-10 text-center shadow-lg">
            <p className="text-xl text-gray-700 pb-5">Your cart is empty!</p>
          ...
          <div className="bg-white rounded-lg p-8 shadow-xl">
              ...
              {cart.map((item) => (
                <li key={item.id} className="flex items-center justify-between bg-gray-50 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600">{item.price === 'Free' ? 'Free' : `Rs. ${item.price}`}</p>
                  </div>
            ...
            <div className="mt-8 flex flex-col items-center">
              <div className="bg-gray-50 text-center p-6 rounded-xl shadow-lg w-full max-w-md">
                <p className="text-2xl font-semibold text-gray-800">Total: <span className="text-red-600">Rs. {total.toLocaleString()}</span></p>
              </div>

              <p className="text-xl text-gray-800 mt-8">Book your class at just 2,000 through e-sewa</p>
              <NavLink to="/payment" className="mt-4 py-2 px-6  text-white font-semibold rounded-full hover:bg-blue-800 transition-colors">
                <img src={esewa} alt="Esewa" className="w-32 mx-auto"/>
              </NavLink>

              <p className=" bg-gray-100 text-center text-gray-600 text-lg mt-4">Or pay on cash at the center</p>
            </div>
          </div>
        )}
      </div>
