...
const Courses = ({ cart, setCart, wishlist, setWishlist }) => {
  ...
  const addToWishlist = (course) => {
      ...
      if (isAlreadyInWishlist) {
        ...
        const updatedWishlist = prevWishlist.filter((item) => item.id !== course.id);
        alert(`${course.name} removed from wishlist!`);
        return updatedWishlist;
      } else {
        // Add the course to the wishlist
        const updatedWishlist = [...prevWishlist, course];
                ...
                >
                  ...
                  <div className="flex flex-row items-center justify-center gap-1 mt-4">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center bg-yellow-400 text-blue-950 py-2 px-6 ml-6 rounded-full hover:bg-yellow-500 transition duration-300 w-40"
                    >
                  ...
                  <NavLink to={`/course/${course.id}`} className="mt-4 w-full">
                    <button className="text-blue-900 text-xl border-2 border-blue-900 rounded-full py-2 w-44 transition duration-300 hover:bg-blue-900 hover:text-white">
                      View Course
                    </button>
                  </NavLink>
                </div>
