...
function Courses() {
  ...
  const courseList = [
    ...
    { name: 'Public Speaking & Presentation', image: project, duration: '1 Month', price: 'Free', instructor: 'Jordan', category: 'Free Courses', id: 'public-speaking' },
    { name: '3D Modeling & Animation', image: 3d, duration: '3 Months', price: '10,000', instructor: 'Ethan', category: 'Creative Arts Courses', id: '3d-modeling' },
  ];
          ...
          {filteredCourses.map((course, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-lg flex flex-col items-center">
              ...
              <NavLink to={`/course/${course.id}`} className='mt-4 w-full'>
                <button className='text-blue-900 border-2 border-blue-900 py-2 px-6 rounded-full w-full hover:bg-gray-200 transition duration-300'>
                  View Course
                </button>
              </NavLink>
