...
const Home = () => {
  ...
  const addToCart = (course) => {
    console.log('Current cart:', cart);  // To check the current cart contents
    const courseExists = cart.some(item => JSON.stringify(item) === JSON.stringify(course));
  ...
  const addToCart = (course) => {
    // Check if course with the same ID is already in the cart
...
{/* Statistics Section */}
<div className="flex flex-wrap justify-center gap-10 py-10 bg-white rounded-lg">
...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 pl-20 pr-20 pt-10 pb-8 bg-gray-200 px-6">
  {courseList.map((course) => (
    <div
      key={course.id}
      className="bg-white p-6 rounded-2xl shadow-lg shadow-blue-900 flex flex-col items-center max-w-xs transform transition duration-300 hover:scale-105 hover:shadow-2xl"
    >
      {/* Course Image */}
      <img
        src={course.image}
