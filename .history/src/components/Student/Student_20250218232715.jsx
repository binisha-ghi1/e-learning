    ...
    <div className="text-center mb-20">
      <div className="flex items-center justify-center gap-6 flex-wrap">
        {students.map((student, index) => (
          <div
            key={index}
            className="flex flex-col bg-gray-200 font-medium p-4 rounded-lg transition-transform transform hover:scale-105 hover:shadow-lg w-72" // Adjusted width to make it smaller
          >
            <img
              src={student.image}
              alt={`${student.name}'s photo`}
              className="w-32 h-32 mx-auto mb-4 relative bottom-10" // Adjusted image position with relative bottom
            />
