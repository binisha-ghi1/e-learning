...
const Navbar = ({ cart, wishlist, user, handleLogout }) => {
  ...
  // Handle profile dropdown actions (View Profile or Logout)
      ...
      <div className="flex gap-4">
        {!user ? (
          ...
          <div className="relative">
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-2 text-blue-950 font-bold">
              {user.name || 'Profile'} <FaChevronDown className="text-sm" />
            </button>

            {isDropdownOpen && (
              <div className="absolute left-0 mt-2 w-48 bg-white border rounded-lg shadow-lg">
                <button
                  onClick={() => handleProfileAction('viewProfile')}
