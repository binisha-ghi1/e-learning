const Navbar = ({ cart, wishlist, user, handleLogout }) => {
  ...
  return (
    <div className="flex flex-row flex-wrap bg-yellow-400 dark:bg-gray-900 text-xl sticky top-0 z-50 font-oswald py-4 px-6 items-center justify-between">
      ...
      {/* Dark Mode Toggle */}
      <button onClick={toggleDarkMode} className="text-2xl text-blue-950 dark:text-yellow-400">
      ...
      <div className="flex items-center gap-4">
        <NavLink to="/wishlist" className="text-2xl text-blue-950 dark:text-yellow-400">
          <FaRegHeart />
          {wishlist?.length > 0 && <span className="text-sm">({wishlist.length})</span>}
        </NavLink>
        <NavLink to="/cart" className="text-2xl text-blue-950 dark:text-yellow-400">
          <IoCartOutline />
          {cart?.length > 0 && <span className="text-sm">({cart.length})</span>}
        </NavLink>
      </div>
    </div>
  );
