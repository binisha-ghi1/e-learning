  ...
  return (
    <div className="flex flex-row flex-wrap bg-yellow-400 text-xl sticky top-0 z-50 font-oswald py-4 px-6 items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <NavLink to="/wishlist" className="text-2xl absolute text-blue-950">
          <FaRegHeart />
          {wishlist?.length > 0 && <span className="text-sm font-medium text-red-600">({wishlist.length})</span>}
        </NavLink>
        <NavLink to="/cart" className="text-2xl absolute text-blue-950">
          <IoCartOutline />
          {cart?.length > 0 && <span className="text-sm font-medium text-red-600">({cart.length})</span>}
        </NavLink>
      </div>
    </div>
  );
