...
const Login = () => {
 // Destructure properly from context
  ...
  const [isSigningIn, setIsSigningIn] = useState(false);  const { userLoggedIn, login } = useAuth(); 
  ...
  const onSubmit = async (e) => {
    ...
    if (!isSigningIn) {
      ...
      try {
        ...
        login();  // Mock login action, set userLoggedIn to true
      } catch (error) {
  ...
  return (
    <div className="flex h-screen items-center justify-center bg-white">
      ...
      <div className="flex w-4/5 max-w-4xl rounded-lg overflow-hidden shadow-lg shadow-gray-400">
        ...
        >
          <img src={logo} alt="Logo" className="bg-white mt-12 rounded-lg mr-4 flex flex-row relative -top-30 left-35 justify-items-end" />
          <p className="mr-6 ml-4 relative -top-36 px-2 pl-4 italic left-46">S.T. TECH</p>
          <h2 className="text-2xl relative bottom-25 font-semibold text-center">
            Empower Your Learning Journey- <br /> Welcome to S.T. Tech!
          </h2>
          <img src={logsign} alt="Sign In Illustration" className="w-54 relative bottom-8 left-24" />
        </div>
      </div>
    </div>
  );
